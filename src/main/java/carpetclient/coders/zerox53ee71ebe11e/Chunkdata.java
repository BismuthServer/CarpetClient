...
public class Chunkdata implements Serializable {
    ...
    private boolean unpackNBT(NbtCompound nbt) {
        int [] data = nbt.getIntArray("data");
        int size = nbt.getInt("size");
        int time = nbt.getInt("time");
        int offset = nbt.getInt("offset");
    ...
    private void unpackNBTStackTrace(NbtCompound nbt) {
        NbtList nbttaglist = nbt.getList("stackList", 10);
