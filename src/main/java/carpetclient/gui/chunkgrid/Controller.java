...
public class Controller {
    ...
    public void buttonDown(int x, int y, int button) {
        if (button == 0) {
            ...
            Minecraft.getInstance().openScreen(new GuiChunkGridChunk(String.format("Chunk (%d, %d)  Gametick: %d", cx, cz, lastGametick), debug, debug, props, stacktrace.size() != 0 ? stacktrace : null));
        }else if(button == 2){
            int cx = debug.getChunkGrid().getGridX(x) + view.getX() - debug.getChunkGrid().sizeX() / 2;
            int cz = debug.getChunkGrid().getGridY(y) + view.getY() - debug.getChunkGrid().sizeZ() / 2;
            String s = "/tp " + (cx*16+8) + " ~ " + (cz*16+8);
            Screen.setClipboard(s);
        }
    }
    ...
    public static void tick() {
        if (Minecraft.getInstance().screen instanceof GuiChunkGrid) {
            Controller c = ((GuiChunkGrid) Minecraft.getInstance().screen).getController();
            if (c.play) {
